...
type ResizeHandlesProps = {
  corners?: Corner[];
  onPointerDown?: (e: React.PointerEvent, c: Corner) => void;
  radio?: number;
  grosor?: number;
...
const ResizeHandles: React.FC<ResizeHandlesProps> = ({
  ...
  const cornerMap: Record<Corner, any> = {
    se: {
      right: 20,
      bottom: 20,
      cursor: 'se-resize',
  ...
  const handleOpacity = Number.parseFloat(getGlobalStyle('--opacity-25')) || 0.25;

  const [hoverMap, setHoverMap] = useState<Record<string, { hovering: boolean; marker: null | { x: number; y: number } }>>(
    () =>
      corners.reduce((acc, c) => {
        acc[c] = { hovering: false, marker: null };
        return acc;
      }, {} as Record<string, { hovering: boolean; marker: null | { x: number; y: number } }>)
  );
  ...
  useEffect(() => {
    setHoverMap((prev) => {
      ...
      // Remove corners that are no longer present
      Object.keys(next).forEach((key) => {
        if (!corners.includes(key as Corner)) {
          delete next[key];
        }
      });
      return next;
